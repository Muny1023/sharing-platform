...
import { aiChat, get, put, remove } from '@/net'
...
function summarizeUnread(n: Notice) {
  if (!n.postId || feedbackLoading.value !== null) return
  feedbackLoading.value = n.postId
  aiChat({ message: '概括这个帖子未读评论的反馈', feedbackPostId: n.postId, unreadOnly: true }, r => { feedbacks.value[n.postId!] = r.data?.data?.reply || '暂无可分析反馈'; feedbackLoading.value = null }, r => { feedbackLoading.value = null; ElMessage.error(r.data?.message || '未读反馈概括失败') }, () => { feedbackLoading.value = null; ElMessage.error('AI 服务暂时没有响应') })
}
onMounted(load)
</script>
...
<template>
  <div class="page"><header><strong @click="router.push('/index')">资源分享社区</strong><button @click="router.push('/index')">返回帖子流</button></header>
    <main><div class="title-row"><h1>消息</h1><div class="actions"><button :class="{active:filter==='all'}" @click="switchFilter('all')">全部</button><button :class="{active:filter==='unread'}" @click="switchFilter('unread')">未读<span v-if="page.unread">（{{page.unread}}）</span></button><button @click="markAll">全部已读</button><button @click="clearRead">清空已读</button></div></div>
      ...
      <article v-for="n in notices" :key="n.id" :class="{unread:!n.read}" @click="markRead(n)"><div><strong>{{n.actorNickname}}</strong> {{n.content}}</div><small>{{n.createdAt}}</small><p v-if="n.postDeleted" class="deleted">相关帖子已删除</p><p v-else-if="n.commentDeleted" class="deleted">相关评论已删除</p><p v-if="feedbacks[n.postId!]" class="feedback"><strong>未读反馈概括：</strong>{{feedbacks[n.postId!]}}</p><button v-if="!n.read && n.postId && !n.postDeleted && (n.type==='POST_COMMENT'||n.type==='COMMENT_REPLY')" class="feedback-btn" :disabled="feedbackLoading!==null" @click.stop="summarizeUnread(n)">{{feedbackLoading===n.postId?'概括中...':'概括该帖未读反馈'}}</button><button class="delete" @click.stop="deleteOne(n)">删除</button></article>
